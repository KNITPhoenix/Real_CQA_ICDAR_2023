    ...
    def __getitem__(self, idx):
        ...
        if self.outputs is not None:
            ...
            if self.args.classifier:
                ...
                #     a = float(answer)
                #     print("answer:",a)
                #     print("target ids:",self.tokenizer.encode(answer, max_length=100, truncation=True))
                # except:
                #     pass

                target_ids = self.tokenizer.encode(answer, max_length=100, truncation=True) #Changed 10 to 100
                # print("decoded: ",self.tokenizer.decode(target_ids,skip_special_tokens = False,clean_up_tokenization_spaces = False))

                out_dict['target_ids'] = torch.LongTensor(target_ids)
                out_dict['target_length'] = len(target_ids)
                # print("output_dictionary:",out_dict)
